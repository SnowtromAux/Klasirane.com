...
const New = (props) => {
  ...
  useEffect(() => {
      const fetchTitleData = async () => {
        try {
          const response = await fetch(`http://localhost:3001/${page}/new/title/${path}/${compName}`);
      ...
      const fetchDescriptionData = async () => {
        try {
          const response = await fetch(`http://localhost:3001/${page}/new/description/${path}/${compName}`);
      ...
      const fetchImgData = async () => {
        try {
          const response = await fetch(`http://localhost:3001/${page}/new/logo/${path}/${compName}`);
      ...
      const fetchAltData = async () => {
        try {
          const response = await fetch(`http://localhost:3001/${page}/new/alt/${path}/${compName}`);
