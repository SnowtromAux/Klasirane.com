...
const Klasirane = (props) => {
  ...
  useEffect(() => {
      const fetchTitleData = async () => {
        try {
          const response = await fetch(`http://localhost:3001/${page}/klasirane/title/${path}/${compName}`);
      ...
      const fetchDescriptionData = async () => {
        try {
          const response = await fetch(`http://localhost:3001/${page}/klasirane/description/${path}/${compName}`);
      ...
      const fetchImgData = async () => {
        try {
          const response = await fetch(`http://localhost:3001/${page}/klasirane/logo/${path}/${compName}`);
      ...
      const fetchMailTextData = async () => {
        try {
          const response = await fetch(`http://localhost:3001/${page}/klasirane/mailData/${path}/${compName}`);
